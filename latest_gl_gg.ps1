...
function Gestion_GL {
    # Créer un OU "Groupes" pour placer les groupes locaux à l'intérieur
    ...
    $gl_ou_path = "OU=gl,OU=Groupes,DC=belgique,DC=lan"

    # Check si l'OU "gl" existe déjà
    if (Get-ADOrganizationalUnit -Filter {Name -eq "gl"} -SearchBase "OU=Groupes,DC=belgique,DC=lan" -ErrorAction SilentlyContinue) {
        Write-Output "L'OU 'gl' existe déjà"
    } else {
        New-ADOrganizationalUnit -Name "gl" -Path "OU=Groupes,DC=belgique,DC=lan"
        Write-Output "L'OU 'gl' a été créée"
    }

    # Ajouter les GL pour chaque SOUS-OU
    foreach ($line in $csv) {
        $ou = $line.ou
        $gl_name = "GL_${ou}"

        # Check si le GL existe déjà
...
function Gestion_Users {
    foreach ($line in $csv) {
        ...
        try {
            New-ADUser -Name $nom -GivenName $prenom -UserPrincipalName $logon_name -Description $description -Office $bureau -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -Enabled $true -Path $ou_path -SamAccountName $nom
